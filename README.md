...
### 1. 技师跳过质检直接交付
- **场景**：技师在「维修中」状态直接尝试交付
- **预期结果**：操作被拒绝，提示「技师不能跳过质检直接交付，请先提交质检」

### 2. 店员越权复核
- **场景**：质检员在质检时选择「通过」，系统会校验
- **预期结果**：质检通过后状态仍为「待质检」，只有店员可以执行交付操作

### 3. 版本冲突拦截
- **场景**：两人同时打开同一张工单，A 先提交修改，B 后提交
- **预期结果**：B 的提交被拒绝，提示版本冲突，需要刷新后重试
...
| quality_check | 待质检 | delivered, quality_failed |
